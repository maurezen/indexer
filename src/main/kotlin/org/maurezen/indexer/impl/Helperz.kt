...
/**
 * Returns a sequence that inserts an element of the other sequence between each two consecutive elements of this sequence
 *
 * The operation is _intermediate_
 */
fun <T> Sequence<T>.mixin(other: Sequence<T>): Sequence<T> {
    val first = iterator()
    ...
    return Sequence { object: Iterator<T> {
        ...
        override fun hasNext(): Boolean {
            return (pickFromFirst && first.hasNext())
                    || (!pickFromFirst && second.hasNext())
        }
